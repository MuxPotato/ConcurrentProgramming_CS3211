...
const RAYON_PERCENT: usize = 90; // % of cores for Rayon

fn main() -> Result<(), Box<dyn Error>> {

    let total_cores = num_cpus::get();       
    let rayon_threads = ((total_cores * RAYON_PERCENT) / 100).max(1);
    let tokio_threads = (total_cores - rayon_threads).max(1);
    ...
    rt.spawn(async move {
        let _ = Server.start_server(first_address, tx).await;
    });
